...
int main(int argc, char* argv[])
{
	...
	MakeMoveTable move;
#ifdef __SIMULATIONMODE__
	solver.Simulation();
#endif
	
#ifdef __TESTMODE__
	cout << "start to read table" << endl;
	solver.ReadTable();
	cout << "end to read table" << endl;
	for (;;) {
		int b[2][3] = {};
		for (int i = 0; i < 2; i++)
			for (int j = 0; j < 3; j++)
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 3; j++) {
				b[i][j] = solver.mapFibOrder[b[i][j]];
			}
		}
		printf(" %6.8f \n", solver.evaluteExpectedScore(b));
		//cout << solver.evaluteExpectedScore(b) << endl;
	}
